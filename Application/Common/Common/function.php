...
function bjw_json($code,$message='操作成功',$data = []){
    if($code == '1' && $message=='操作成功'){
        echo json_encode(array('code'=>"$code",'message'=>"操作失败",'data'=>$data));exit;
    }else{
        echo json_encode(array('code'=>"$code",'message'=>"$message",'data'=>$data));exit;
    }
}
...
function lvd_substr($strs, $startIndex = 0, $strLength, $after_str = '...', $strCode = 'UTF-8') {
	if ($strCode == 'UTF-8') {
		$pa = "/[\x01-\x7f]|[\xc2-\xdf][\x80-\xbf]|\xe0[\xa0-\xbf][\x80-\xbf]|[\xe1-\xef][\x80-\xbf][\x80-\xbf]|\xf0[\x90-\xbf][\x80-\xbf][\x80-\xbf]|[\xf1-\xf7][\x80-\xbf][\x80-\xbf][\x80-\xbf]/";
		preg_match_all($pa, $strs, $t_string);

		if (count($t_string[0]) - $startIndex > $strLength)
			return join("", array_slice($t_string[0], $startIndex, $strLength)) . $after_str;
...
/*电话号码*/
function verifysms($phonenumber) {
...
function verifycode($code) {
	$email = I('post.email');
	$se = session('code');
	if ($se['code'] == $code && $email == $se['email']) {
		return true;
	} else {
		return false;
	}
...
function customerror($error_level) {
